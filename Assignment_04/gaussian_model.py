...
class GaussianModel(nn.Module):
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        # Compute covariance
        # Use clamped scales for numerical stability
        scales = torch.exp(self.scales.clamp(min=-4.0, max=2.0))
        S = torch.diag_embed(scales)

        M = R @ S
        Covs3d = M @ M.transpose(-1, -2)

        # Force symmetry and add diagonal epsilon
        Covs3d = 0.5 * (Covs3d + Covs3d.transpose(-1, -2))
        eps = 1e-4
        eye = torch.eye(3, device=Covs3d.device, dtype=Covs3d.dtype).unsqueeze(0)
        Covs3d = Covs3d + eps * eye
