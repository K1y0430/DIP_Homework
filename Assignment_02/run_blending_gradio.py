...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    fg_mask_bool = foreground_mask.bool().expand_as(fg_lap)
    bg_mask_bool = background_mask.bool().expand_as(blend_lap)

    fg_vals = fg_lap[fg_mask_bool]
    bg_vals = blend_lap[bg_mask_bool]

    loss = torch.mean((fg_vals - bg_vals) ** 2)
